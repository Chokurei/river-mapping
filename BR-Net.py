...
from models import brnet
from utils.runner import brTrainer
from config import GUO
...
class BRNet(object):
    ...
    def __init__(self, args):
        self.model = brnet.BRNetv0(args.in_ch, args.out_ch, args.base_kernel)
...
def main(args):
    ...
    # initialize runner
    run = brTrainer(args, method, is_multi=True)

    print("Start training {}...".format(method))
    run.training(net, [datasets.trainLSE, datasets.val])
    run.save_log()
    run.learning_curve()
