...
def cls_to_img(img_cls, refs):
    ...
    for cls in range(nb_class):
        if type(refs) == pd.core.frame.DataFrame: 
            refs = refs.values
        cls_rgb = refs[cls, 1:]
...
def img_to_slices(img, img_rows, img_cols, border_remain = False):
    """
    ...
    slices = []
    # generate img slices
    # TODO: border_remain, cannot work
    if border_remain:
        iter_rows = nb_rows - 1
        iter_cols = nb_cols - 1
        for i, j in itertools.product(range(nb_rows), range(nb_cols)):
            if i < iter_rows:
                if j < iter_cols:
                    _slice = img[i*img_rows:(i+1)*img_rows,
        ...
        for i, j in itertools.product(range(nb_rows), range(nb_cols)):
            _slice = img[i * img_rows: i * img_rows + img_rows,
                        j * img_cols:j * img_cols + img_cols]
            slices.append(_slice)
    return slices, [nb_rows, nb_cols]
            

def slices_to_img(slices, shapes, border_remain = False):
    """
    ...
    # set img placeholder
        ...
        if len(slices[0].shape) > 2:
            img_rows, img_cols, in_ch = slices[0].shape
            img = np.zeros((img_rows*(shapes[0]-1)+slices[-1].shape[0], \
                            img_cols*(shapes[1]-1)+slices[-1].shape[1], \
                            in_ch), np.uint8)
        else:
            img_rows, img_cols = slices[0].shape
            img = np.zeros((img_rows*(shapes[0]-1)+slices[-1].shape[0], \
                            img_cols*(shapes[1]-1)+slices[-1].shape[1]), np.uint8)
        
        for i, j in itertools.product(range(shapes[0]), range(shapes[1])):
            if i != shapes[0] - 1:
                if j != shapes[1] - 1:
                    img[i*img_rows:(i+1)*img_rows,
                        j*img_cols:(j+1)*img_cols] = slices[i*shapes[1]+j]
                else:
                    img[i*img_rows:(i+1)*img_rows,
                        j*img_cols] = slices[i*shapes[1]+j]
            else:
                if j != shapes[1] - 1:
                    img[i*img_rows:,
                        j*img_cols:(j+1)*img_cols] = slices[i*shapes[1]+j]
...
def pair_to_rgb(gen_img, tar_img, background='black', use_dilation=False, disk_value=2):
    ...
    if background == "black":
        ...
        rgb_img[:, :, 2][np.logical_and(gen_img == 255, tar_img == 0)] = 255
        ...
        rgb_img[:, :, 0][np.logical_and(gen_img == 0, tar_img == 255)] = 255
...
def shift_edge(img, value=1, dtype="uint8"):
    ...
    img_shift_l[:, value:] = img[:, :-value]
    img_shift_d[value:, :] = img[:-value, :]
